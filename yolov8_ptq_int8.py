...
def export_onnx(model, onnx_filename, batch_onnx, dynamic_shape, simplify, imgsz=640, prefix=colorstr('calib: ')):
    ...
    dummy_input = torch.randn(batch_onnx, 3, 736, 2560)
    ...
    try:
        ...
        with torch.no_grad():
            torch.onnx.export(model.cpu(), 
                            ...
                            input_names=['images'],
                            output_names=['output'],
                            dynamic_axes={'images': {0: 'batch'}} if dynamic_shape else None,
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default='/home/jia/PycharmProjects/yolov8-pytorch_quantization/data/dataset_basketball.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default="/home/jia/PycharmProjects/yolov10/runs/detect/basketball_yolov8s_relu/weights/best.pt", help='model.pt path(s)')
    parser.add_argument('--model-name', '-m', default='yolov8s', help='model name: default yolov5s')
    ...
    parser.add_argument('--sensitive-layer', default=None,
                        help='skip sensitive layer: default detect head')
...
if __name__ == "__main__":
    ...
    #     res = yolo.val(data=opt.data)
    calib_output = os.path.join(opt.out_dir, F"{opt.model_name}-max-{opt.num_calib_batch * data_loader.batch_size}.pt")
    yolo.save(calib_output)
    # onnx_filename = 'yolov8s_ptq_detect.onnx'
